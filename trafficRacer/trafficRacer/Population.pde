class Population {
  ...
  AiCar bestPlayer;//the best ever player 
  ...
  void updateAlive() {
    ...
    for (int i = 0; i< pop.size(); i++) {
      ...
      if (!pop.get(i).dead) {
        ...
        pop.get(i).doStuff();//use outputs from neural network
        pop.get(i).show();
      }
    }
  }
  ...
  void naturalSelection() {
    ...
    ArrayList<AiCar> children = new ArrayList<AiCar>();//the next generation
    println("Species:");               
